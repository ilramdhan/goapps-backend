...
// mbTypeCode is the cost_product_type.cpt_type_code reserved for MB recipe products
// (seeded by migration 000450). Products of this type are auto-generated by the MB
// Recipe workflow and must never be created through the product-master API.
const mbTypeCode = "MB"
...
// MB product type. An unresolvable type is passed through: domain.New / the FK reject it.
func (h *CreateHandler) rejectMBType(ctx context.Context, productTypeID int32) error {
	if h.typeRepo == nil || productTypeID <= 0 {
		return nil
	}
	pt, err := h.typeRepo.GetByID(ctx, productTypeID)
	if err != nil {
	...
	if pt.TypeCode() == mbTypeCode {
		return domain.ErrMBProductNotManuallyCreatable
	}
	return nil
}

// Handle executes the create. MB-typed products are refused: they exist only as the
// auto-generated output of the MB Recipe workflow.
func (h *CreateHandler) Handle(ctx context.Context, cmd CreateCommand) (*domain.CostProductMaster, error) {
	if err := h.rejectMBType(ctx, cmd.ProductTypeID); err != nil {
		return nil, err
	}
