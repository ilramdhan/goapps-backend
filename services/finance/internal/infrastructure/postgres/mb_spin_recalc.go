...
// one row per child (plan §P8 "Jejak"). The row is written even when Updates is
// empty, because "this operation was eligible to change nothing" is itself the
// fact the audit trail must keep.
//
// ⛔ The UPDATE re-asserts the candidate predicate in its WHERE clause
// (status = 'R and D', not deleted, parent matches). The caller already filtered,
// but a row that changed status between the read and the write must NOT be
// overwritten — A7 is enforced twice on purpose, and the second time is the one
