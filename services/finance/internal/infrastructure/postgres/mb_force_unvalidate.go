...
//  2. resets mbh_cost_product_id/mbh_cost_generated_at/mbh_cost_generated_by to NULL so a
//     subsequent Bulk Validate takes the FULL autoGenCostProduct path instead of the lighter
//     regenerateCostProductRMs path (see mb_autogen_repository.go's TransitionWithAutoGen);
...
func (r *MBHeadRepository) forceUnvalidateUpdateTx(ctx context.Context, tx *sql.Tx, id uuid.UUID, stateReason, actorUserID string) error {
	const q = `
		...
		SET mbh_entry_status = $2,
		    ...
		    mbh_cost_product_id = NULL,
		    mbh_cost_generated_at = NULL,
		    mbh_cost_generated_by = NULL,
		    updated_at = NOW(),
		    updated_by = NULLIF($4, '')
